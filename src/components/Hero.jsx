...
const Hero = () => {
  ...
  const [isMuted, setIsMuted] = useState(false); // Try to autoplay with sound
  ...
  useEffect(() => {
    ...
    // Pause video when user scrolls down to focus on reading
    const handleScroll = () => {
  ...
  // Looping Typewriter effect for the title highlight
  useEffect(() => {
    const phrases = [
  ...
  // Update state when video naturally finishes
  useEffect(() => {
  ...
  const toggleVideo = (e) => {
    ...
    if (videoRef.current) {
      if (!hasInteracted) {
        // First manual interaction: Restart video and unmute
        ...
        // Subsequent interactions: Normal play/pause
  ...
  return (
    <section className="relative w-full h-screen overflow-hidden bg-black">
      ...
      <video
        ...
        playsInline
        poster={heroPoster}
        // Removed 'loop' so it plays only once
      ...
      <div className="absolute inset-0 z-20 px-6 pb-20 md:pb-[8%] md:px-12 max-w-7xl mx-auto flex flex-col md:flex-row justify-end md:justify-between items-start md:items-end text-left w-full">
        ...
        <div className="flex flex-col items-start text-left max-w-2xl w-full">
          ...
          >
            ...
            <span className="text-[#00FF00] font-mono drop-shadow-[0_0_8px_rgba(0,255,0,0.8)]">
              {typewriterText}
          ...
          >
            ...
            <MagneticButton>
              <a 
                ...
                className="px-4 py-2 md:px-6 md:py-2 text-xs md:text-base rounded-full bg-white text-black font-semibold hover:bg-gray-200 transition-all duration-300 transform hover:scale-105 shadow-md block"
              >
                {heroContent.ctaPrimary.text}
              </a>
            </MagneticButton>
            
            {/* Secondary Button - Glassmorphism style (Now Call) */}
            <a 
              href={heroContent.ctaSecondary.href}
              className="px-4 py-2 md:px-6 md:py-2 text-xs md:text-base rounded-full bg-black/40 border border-white text-white font-semibold hover:bg-black/60 transition-all duration-300 backdrop-blur-md flex items-center gap-2"
            >
            ...
            {/* Email Button */}
            <a 
              href={heroContent.ctaEmail.href}
              className="px-4 py-2 md:px-6 md:py-2 text-xs md:text-base rounded-full bg-transparent border border-[#ff2a2a] text-[#ff2a2a] hover:text-white font-semibold hover:bg-[#ff2a2a] transition-all duration-300 backdrop-blur-md flex items-center gap-2"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
              {heroContent.ctaEmail.text}
            </a>
        ...
        >
          ...
          <span className="text-white text-[10px] md:text-xs font-bold tracking-widest uppercase opacity-70 group-hover:opacity-100 transition-opacity">
            {!isPlaying || isMuted ? (hasInteracted ? "Play" : "Play Reel") : "Pause"}
          </span>
        </div>
      </div>
